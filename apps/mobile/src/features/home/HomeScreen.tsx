...
export default function HomeScreen() {
  ...
  return <Screen>
    ...
    <Text style={styles.title}>{t('hi')} {firstName} <Text style={styles.titleDecor}>🌙</Text></Text>
    <Pressable accessibilityRole="button" onPress={() => router.push('/write')} style={styles.cta}><Text style={styles.ctaTitle}>{t('homeWrite')}</Text><Text style={styles.ctaBody}>{t('homeWriteBody')}</Text><Text style={styles.ctaAction}>{t('openEditor')}</Text></Pressable>
    {error ? <Text accessibilityRole="alert" style={styles.error}>{error}</Text> : null}
    {isLoading ? <View style={styles.loading}><ActivityIndicator color={colors.coralDark} /></View> : <>
      <View style={styles.statsRow}><Stat value={stats?.totalEntries ?? 0} label={t('pagesSealed')} /><Stat value={stats?.activeCircles ?? 0} label={t('activeCircles')} /><Stat value={stats?.bloomedCircles ?? 0} label={t('bloomed')} /></View>
      <Text style={styles.section}>{t('comingUp')}</Text>
      {upcoming.length === 0 ? <EmptyCard onPress={() => router.push('/circles')} title={t('noSealed')} body={t('plantFirst')} /> : upcoming.map(circle => <CircleCard key={circle.id} circle={circle} onPress={() => router.push({ pathname: '/circle-detail/[circleId]', params: { circleId: circle.id } })} />)}
      {bloomed.length > 0 ? <><Text style={styles.section}>{t('readyToOpen')}</Text>{bloomed.map(circle => <CircleCard key={circle.id} circle={circle} onPress={() => router.push({ pathname: '/circle/[circleId]', params: { circleId: circle.id } })} />)}</> : null}
    </>}
  </Screen>;
}
...
function greeting(t: (key: 'morning' | 'afternoon' | 'evening') => string): string { const hour = new Date().getHours(); return hour < 12 ? t('morning') : hour < 18 ? t('afternoon') : t('evening'); }
